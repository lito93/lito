      ...
      {selected && (
        <div onClick={() => onDealClick(selected)} style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: th.sub, fontWeight: 600, marginTop: 3 }}>
              ⏰ {daysLeftLabel(selected.expiryDate, lang)} {tx.left}
            </div>
          </div>
          <div style={{ background: selected.color, color: "#fff", borderRadius: 10, padding: "6px 12px", fontWeight: 800, fontSize: 16 }}>
            -{selected.discount}%
          </div>
        </div>
      )}
            ...
            {p.params.map((row, idx) => (
              <div key={idx} style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
                ...
                <button onClick={() => update({ params: p.params.filter((_, i) => i !== idx) })}
                  style={{ background: "#FFF0F0", border: "none", borderRadius: 10, width: 38, height: 38, color: "#E63946", fontSize: 16, cursor: "pointer", flexShrink: 0 }}>✕</button>
              </div>
            ))}
